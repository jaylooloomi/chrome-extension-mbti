...
export const analyzeMBTI = async (apiKey: string, bookmarkStructure: any, language: string): Promise<MBTIResult> => {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      logger.error(`Gemini Analysis Error (Attempt ${attempt}/${maxRetries}):`, error);
      ...
      logger.debug(`Rate limit detected. Retrying in ${delay}ms...`);
  ...
  throw new Error("Failed to analyze MBTI after multiple attempts.");
};
